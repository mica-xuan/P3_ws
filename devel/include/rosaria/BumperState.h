...
struct BumperState_
{
  ...
  _header_type header;
   ...
   typedef std::vector<uint8_t, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<uint8_t>> _front_bumpers_type;
  _front_bumpers_type front_bumpers;

   typedef std::vector<uint8_t, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<uint8_t>> _rear_bumpers_type;
...
struct Printer< ::rosaria::BumperState_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::rosaria::BumperState_<ContainerAllocator>& v)
  {
    ...
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "front_bumpers: ";
    if (v.front_bumpers.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.front_bumpers.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint8_t>::stream(s, true ? std::string() : indent + "    ", v.front_bumpers[i]);
    }
    if (v.front_bumpers.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "rear_bumpers: ";
    if (v.rear_bumpers.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.rear_bumpers.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint8_t>::stream(s, true ? std::string() : indent + "    ", v.rear_bumpers[i]);
    }
    if (v.rear_bumpers.empty() || true)
      s << "]";
  }
